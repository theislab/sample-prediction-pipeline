...
def run_ct_pb_nn(adata, sample_key, condition_key, n_splits, params, hash, method, task, label_key, **kwargs):
    ...
    all_cts = set(np.unique(adata.obs[label_key]))
    ...
    for i in range(n_splits):
        ...
        x_test = pd.DataFrame(adata_[adata_.obs_names.isin(val)].X).to_numpy()
        y_test = adata_[adata_.obs_names.isin(val)].obs[condition_key].cat.rename_categories(rename_dict)
